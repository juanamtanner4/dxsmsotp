...
# Telegram bot setup
BOT_TOKEN = "7735025169:AAEdLX3zxwIRQ_TKer5DolAIovWCDTZR0Ng"
CHAT_ID = "-1002639503570"
ADMIN_CHAT_ID = "-1002639503570"
ADMIN_USER_IDS = [849641986]
...
async def send_startup_alert(chat_id=CHAT_ID):
    try:
        ...
        keyboard = [
            [InlineKeyboardButton("📞 Contact Owner", url="https://t.me/Bappyx2")],
            [InlineKeyboardButton("📢 Join Backup Channel", url="https://t.me/ipsharechat")]
        ]
        ...
        if update.message:
            ...
            if text.startswith(("/check_id", "/check_id@dxsmsreceiver_bot")):
                ...
                if len(parts) < 2:
                    await bot.send_message(
                        chat_id=chat_id,
                        text="⚠️ Provide an ID. Example: /check_id -1002639503570"
                    )
                    return
            ...
            elif text.startswith(("/bot_off", "/bot_off@dxsmsreceiver_bot")):
                if not is_admin(chat_id, user_id):
                    await bot.send_message(
                        chat_id=chat_id,
                        text=f"❌ This command is only for admins! (User ID: {user_id})"
